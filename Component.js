export class Component {
  ...
  async parseAttributes(attributes, node) {
    const o = {};
    await Promise.all(
      Array.from(attributes).map(async (a) => {
        ...
        switch (a.name[0]) {
          ...
          case ":":
            ...
            await this.replaceAttributes(node, a.name, name1, await this._get_value(a.value))
            break;
          default:
            // prop
            o[a.name] = a.value;
      ...
      switch (node.nodeType) {
        case 1: // tag
          const tag = node.tagName.toLowerCase(),
            ...
            props = await this.parseAttributes(node.attributes, node);
